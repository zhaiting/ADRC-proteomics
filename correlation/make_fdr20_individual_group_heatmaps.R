...
individual_layers <- tibble::tribble(
  ~discovery_source, ~base_dir,
  "All",             file.path(out_base, "dream_individual_baseAge55"),
  "Female",          file.path(out_base, "dream_individual_baseAge55_sexF"),
  "Male",            file.path(out_base, "dream_individual_baseAge55_sexM")
)

individual_contrasts <- tibble::tribble(
  ~contrast,       ~file,                        ~contrast_title,
  "AD_vs_HC",      "toptable_AD_vs_HC.csv",      "AD vs HC",
  "MCI_vs_HC",     "toptable_MCI_vs_HC.csv",     "MCI vs HC",
  "PD_vs_HC",      "toptable_PD_vs_HC.csv",      "PD vs HC",
  "MCI_PD_vs_HC",  "toptable_MCI_PD_vs_HC.csv",  "MCI-PD vs HC",
  "LBD_vs_HC",     "toptable_LBD_vs_HC.csv",     "LBD vs HC"
)
...
read_layer_contrast <- function(discovery_source, base_dir,
  ...
  if (!file.exists(path)) {
    message("  [skip] missing individual toptable: ", path)
    return(tibble())
  }
...
individual_all_layers <- purrr::pmap_dfr(
  tidyr::crossing(individual_layers, individual_contrasts),
  read_layer_contrast
)

individual_panel_all <- panel_meta %>%
  inner_join(individual_all_layers, by = "SomaKey", relationship = "many-to-many") %>%
...
sex_panel_dir <- file.path(out_base, "dream_individual_baseAge55", "panel_subsets_sexpanels")
dir.create(sex_panel_dir, showWarnings = FALSE, recursive = TRUE)
readr::write_csv(
  individual_panel_all,
  file.path(sex_panel_dir, "dream_panel_subset_all_somakeys.csv")
)

individual_panel_summary <- individual_panel_all %>%
...
fit_one_pair_sex_panel <- function(somakey,
  ...
  df_cc <- df %>%
    ...
    filter(!is.na(y), !is.na(x), if_all(all_of(age_terms), ~ !is.na(.x)),
           !is.na(SubjectID), !is.na(group_individual))
  ...
  if (nlevels(df_cc$group_individual) >= 2) {
...
contrast_colors <- c(
  ...
  "MCI-PD vs HC" = "#009E73",
  "LBD vs HC" = "#D55E00",
  "AD vs HC | LBD vs HC" = "#B0B0B0",
  "LBD vs HC | PD vs HC" = "#B0B0B0",
  "LBD vs HC | MCI-PD vs HC" = "#B0B0B0",
  "MCI-PD vs HC | PD vs HC" = "#B0B0B0"
)

plot_panel <- function(panel_name) {
  candidates <- make_candidates(panel_name)
  if (nrow(candidates) == 0L) {
    warning("No FDR<", DISCOVERY_FDR, " individual sex-panel candidates for ", panel_name)
  ...
  readr::write_csv(
    ...
    file.path(analysis_dir,
              paste0("candidate_roster_", panel_name,
                     "_individual_sexpanels_", DISCOVERY_TAG, "_v2.csv"))
  )
  ...
  out_tag <- paste(panel_name, "individual", "sexpanels", DISCOVERY_TAG, sep = "_")
  readr::write_csv(
    res_lmm,
    file.path(analysis_dir,
  ...
  mx_display <- max(0.30, ceiling(mx * 10) / 10)
  ...
  plot_height <- 5.75
  out_stub <- paste("heatmap", panel_name, "individual", "sexpanels",
...
message("Wrote FDR<", DISCOVERY_FDR, " individual sex-panel heatmaps to: ", out_dir)
